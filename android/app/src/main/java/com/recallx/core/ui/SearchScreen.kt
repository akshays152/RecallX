...
fun SearchScreen(repository: RecallXRepository, onBack: () -> Unit, onOpenMemory: (String) -> Unit) {
    ...
    Column(Modifier.fillMaxSize().padding(horizontal = 20.dp)) {
        ...
        LazyColumn(contentPadding = PaddingValues(bottom = 24.dp), verticalArrangement = Arrangement.spacedBy(14.dp)) {
            ...
            item {
                OutlinedTextField(value = input, onValueChange = { input = it; vm.setQuery(it) }, modifier = Modifier.fillMaxWidth(), placeholder = { Text("Try: Find the hotel screenshot around ₹1,300") }, label = { Text("Memory search") }, singleLine = false, minLines = 2, trailingIcon = { if (input.isNotEmpty()) TextButton(onClick = { input = ""; vm.clearQuery() }) { Text("Clear") } }, keyboardOptions = androidx.compose.foundation.text.KeyboardOptions(imeAction = androidx.compose.ui.text.input.ImeAction.Search), keyboardActions = androidx.compose.foundation.text.KeyboardActions(onSearch = { vm.submit(input) }) )
            }
            if (state.recentQueries.isNotEmpty() && state.phase != SearchPhase.SEARCHING) item { RecentQueries(state.recentQueries, onSelect = { input = it; vm.submit(it) }, onClear = vm::clearRecentSearches) }
            ...
            if (state.phase == SearchPhase.SUCCESS) {
                ...
                itemsIndexed(state.results, key = { _, result -> result.memory.id }) { index, result -> SearchResultCard(result, isTop = index == 0) { onOpenMemory(result.memory.id) } }
            }
        }
    }
}
...
@Composable private fun RecentQueries(queries: List<String>, onSelect: (String) -> Unit, onClear: () -> Unit) { Column(verticalArrangement = Arrangement.spacedBy(8.dp)) { Row(Modifier.fillMaxWidth(), horizontalArrangement = Arrangement.SpaceBetween) { Text("Recent searches", style = MaterialTheme.typography.titleMedium, fontWeight = FontWeight.SemiBold); TextButton(onClick = onClear) { Text("Clear history") } }; Row(Modifier.fillMaxWidth().horizontalScroll(rememberScrollState()), horizontalArrangement = Arrangement.spacedBy(8.dp)) { queries.forEach { FilterChip(selected = false, onClick = { onSelect(it) }, label = { Text(it) }) } } } }
